...
const GoogleButton = ({ textContent, onGoogleSubmit }: GoogleButtonProps) => {
  ...
  const googleText = textContent ? 'Sign up with Google' : 'Continue with Google';
  ...
  return (
    <Button fullWidth variant="contained" onClick={handleGoogleClick}
    sx={googleButtonStructure} startIcon={<GoogleIcon />}>
      {googleText}
    </Button>
  );
};
